...
class VRNN(nn.Module):
    ...
    def __init__(self, x_dim, h_dim, z_dim):
        ...
        # paper: We found that these feature extractors are crucial for learning complex sequences.
        # paper: All of phi_t have four hidden layers using rectified linear units ReLU.
        self.x_emb = nn.Sequential(
            nn.Embedding(num_embeddings=4 ** 4, embedding_dim=h_dim)
        )
    ...
    def forward(self, x):
        ...
        :param x: shape of [frames, samples, features]
        :return:
        """
        ...
        for time in range(x.shape[1]):
            # feature extractor:
            emb_x = self.x_emb(x[:, time])
            phi_x = self.x_fea(emb_x)
    ...
    def sampling(self, seq_len, device):
        ...
        sample = torch.zeros(seq_len, self.h_dim, device=device)
